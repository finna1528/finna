$(function () {
  "use strict";
  $(document).ready($("#btn_open").fadeIn().css("display", "block"));

  // On window's load
  $(window).on("load", function () {
    var aud = document.getElementById("finna_audio");
    populateColorPlates();
    setTimeout(function () {
      $("#btn_open").click(function (e) {
        $(".page_loader .content-loader i.fa").css("transform", "scale(100)");
        $(".page_loader").delay(1250).fadeOut("slow");
        $('link[id="style_sheet"]').attr(
          "href",
          "assets/css/skins/default.css"
        );
        $("audio").attr("autoplay", true).load();
      });
      setDefaultPosition();
    }, 1000);
  ...
  $(function () {
    ...
    var endDate = "September  10, 2022 10:00:00";
      ...
      render: function (data) {
        $(this.el).html(
          "<div>" +
            ...
            this.leadingZeros(data.hours, 1) +
            " <span>Hours</span></div><div>" +
            this.leadingZeros(data.min, 1) +
            " <span>Minutes</span></div><div>" +
            this.leadingZeros(data.sec, 1) +
...
var audio_state = $("audio").data("state");
function pause() {
  $("audio").trigger("pause");
  $("#button_audControls").attr("onclick", "resume()");
  $("#button_audControls i").addClass("fa-circle-play");
  $("#button_audControls i").removeClass("fa-circle-pause");
}

function resume() {
  $("audio").trigger("play");
  $("#button_audControls").attr("onclick", "pause()");
  $("#button_audControls i").addClass("fa-circle-pause");
  $("#button_audControls i").removeClass("fa-circle-play");
}
